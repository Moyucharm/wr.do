...
The `GET /api/v1/email` endpoint returns all email addresses owned by the current account, including their starred state.
...
Results are sorted with starred addresses first, then by creation time descending.
